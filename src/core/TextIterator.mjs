/**
 * This is an iterator over text (tree of strings).
 * It operates on immutable tree, and therefore is never invalidated.
 * Most of the semantics of this iterator are equivalent
 * to the semantics of |text.substring(from, to)|.
 */
export class TextIterator {
  /**
   * @param {!TreeIterator<string>} iterator
   * @param {number} offset
   * @param {number} from
   * @param {number} to
   * @param {number} length
  ...
  substr(length) {
    ...
    let result = '';
    let iterator = length <= this._chunk.length * 2 ? this._iterator : this._iterator.clone();
    let pos = this._pos;
    let moves = -1;
    do {
      ++moves;
